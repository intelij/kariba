...
class UserController extends Controller {
	...
	public function getRegister() {
		...
		if(CNF_REGIST =='false') :    
			if(\Auth::check()):
				 return Redirect::to('')->with('message',\SiteHelpers::alert('success','You are already login'));
			else:
	...
	public function postRequest( Request $request)
	{
		...
		if ($validator->passes()) {
			...
			$clients =  Clients::where('email','=',$request->input('credit_email'));
			...
			if($user->count() >=1)
			{
				...
				$activationLink = "http://karibafiance.com/" . trim('user/reset/'. $request->input('_token'));

				$message = "REQUEST PASSWORD RESET \r\n";

				$message.= "To reset your password, complete this form:\r $activationLink.";

				$message.=  "\n\r[ " .CNF_APPNAME." ]";

				//        TODO: Uncomment this when pushing live
				$twillio->message($client->mobile, $message);
				...
				return Redirect::to('user/login')->with('message', \SiteHelpers::alert('success','Please check your email or SMS'));
