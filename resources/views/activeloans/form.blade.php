...
<div class="sbox">
	...
	<div class="sbox-content"> 	
		...
		<ul class="parsley-error-list">
		 ...
		 {!! Form::open(array('url'=>'activeloans/save?return='.$return, 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ')) !!}
<div class="col-md-12">
						<fieldset><legend> Activeloans</legend>
			...
			<div style="clear:both"></div>	
				  ...
				  <div class="form-group">
					...
					<button type="button" onclick="location.href='{{ URL::to('activeloans?return='.$return) }}' " class="btn btn-warning btn-sm "><i class="icon-cancel-circle2 "></i>  {{ Lang::get('core.sb_cancel') }} </button>
					</div>	  
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#account_state").jCombo("{!! url('activeloans/comboselect?filter=accounts_state:account_state:account_state') !!}",
		{  selected_value : '{{ $row["account_state"] }}' });
		
		$("#product").jCombo("{!! url('activeloans/comboselect?filter=products:id:product') !!}",
		{  selected_value : '{{ $row["product"] }}' });
		 

		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("activeloans/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
