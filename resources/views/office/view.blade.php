...
<div class="sbox ">
	...
	<div class="sbox-tools" >
   		<a href="{{ URL::to('office?return='.$return) }}" class="tips btn btn-xs btn-white pull-right" title="{{ Lang::get('core.btn_back') }}"><i class="icon-backward"></i>&nbsp;{{ Lang::get('core.btn_back') }}</a>
		@if($access['is_add'] ==1)
   		<a href="{{ URL::to('office/update/'.$id.'?return='.$return) }}" class="tips btn btn-xs btn-white pull-right" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-edit"></i>&nbsp;{{ Lang::get('core.btn_edit') }}</a>
		@endif 
	</div>	
	</div>
		...
		<table class="table table-striped table-bordered" >
			<tbody>	
					...
					<tr>
						...
						<td>{{ SiteHelpers::formatLookUp($row->branch,'branch','1:branch:id:name') }} </td>
