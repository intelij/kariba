...
<div class="sbox-tools" >
	...
	<div class="sbox-content"> 	
	    <div class="toolbar-line ">
			...
			@endif 
			<a href="{{ URL::to( 'contracts/search') }}" class="btn btn-sm btn-white" onclick="SximoModal(this.href,'Advance Search'); return false;" title="{{ Lang::get('core.btn_search') }}"><i class="icon-search3"></i> {{ Lang::get('core.btn_search') }} </a>				
			@if($access['is_excel'] ==1)
			<a href="{{ URL::to('contracts/download?return='.$return) }}" class="tips btn btn-sm btn-white" title="{{ Lang::get('core.btn_download') }}">
            ...
            @foreach ($rowData as $row)
                <tr>
				 ...
				 <td>
						...
						<a href="{{ URL::to('contracts/show/'.$row->id.'?return='.$return)}}" class="tips btn btn-xs btn-primary" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-search "></i></a>
						@endif
						@if($access['is_edit'] ==1)
						<a  href="{{ URL::to('contracts/update/'.$row->id.'?return='.$return) }}" class="tips btn btn-xs btn-success" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-edit "></i></a>
						@endif
